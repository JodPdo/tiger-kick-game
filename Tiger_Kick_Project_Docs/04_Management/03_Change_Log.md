...
## [0.53] — 2026-07-19 — TK-P2-15 architect ruling: GameManager stays SCENE-LOCAL (autoload-vs-scene-local open question resolved, no code change)

- **TK-P2-15 (architect ruling) APPROVED — GameManager stays scene-local, NOT promoted to an autoload.** This resolves, non-silently and on the record, the open architecture question carried forward from the original TK-P2-03 architect ruling / design doc §8a: whether `managers/GameManager.gd` must become an autoload to persist match state across the WaitingRoom↔Arena scene transition, or stay scene-local. Ruling: **stay scene-local.** No code change — `gameplay-engineer`'s implemented choice and `code-reviewer`'s concurrence are correct on the merits; this entry provides the required architect blessing + Change Log record (same procedural pattern as `[0.45]`/TK-P2-28, which obtained an explicit ruling even to bless a status-quo path).
- **Verified against the code (architect read `managers/GameManager.gd` in full):** every piece of match-scoped state GameManager holds — `match_state`, `_round_count`, `_tiger_rng`, `_is_tag_sequence_active`, `_countdown_started`, `_first_tiger_assigned`, `_first_tiger_grace_elapsed`, `_match_ended_locally`, `_expected_first_tiger_roster_size` — is either initialised at declaration or (re)built inside the host-only `_ready()` (fresh `_tiger_rng.randomize()`, fresh roster snapshot from a **live** `NetworkManager.get_connected_peer_ids()` call, fresh signal wiring). A fresh `GameManager` node is created with every fresh `TestArena` instance, so all of it resets **for free** on the next arena entry, with zero stale carry-over. `NetworkManager` (the one relevant autoload) persists only the connection/roster and is queried live, never as cached match state — nothing leaks match state across the boundary through it.
- **Both scene-transition boundaries confirmed self-contained:** WaitingRoom→Arena rides the pre-existing, human-verified `ui/WaitingRoom.gd::_rpc_start_match()` broadcast (TK-P2-13); the new Arena→WaitingRoom `MATCH_END`→WaitingRoom edge rides this card's symmetric `return_to_waiting_room()` (`@rpc(authority,call_local,reliable)`, mirrors `_rpc_start_match()`'s idempotency/deferred-switch shape). Neither edge needs a node that survives the transition — both are "host broadcasts a synced scene switch". `MatchStateRules.State.WAITING_ROOM` is a nominal enum member (no live GameManager instance branches on it), present only to keep the state machine complete.
- **Rationale (why scene-local is the more integral choice, not merely adequate):** (1) nothing in the match SM's scope needs cross-boundary persistence, so an autoload would add a project-wide reachable node holding state that is meaningless in MainMenu/WaitingRoom; (2) an autoload would require **explicit per-match reset logic** on every return-to-WaitingRoom to avoid exactly the stale-state class scene-local avoids by construction — i.e. scene-local is *safer*, not just simpler; (3) it keeps GameManager's existing host-authoritative surface (the `is_server()` gates, RPC `call_local` patterns, the reviewed/human-tested Tag Sequence + Safe Circle + first-Tiger paths) untouched, introducing zero risk to already-proven code. Server-authority boundary is fully preserved — every game-deciding transition (`set_match_state`, `assign_first_tiger`, `apply_role_switch`, `return_to_waiting_room`) remains host-broadcast, `is_server()`-gated.
- **Scope of this ruling — correct for Phase 3 as currently scoped, with a defined revisit trigger (NOT a permanent lock):** reviewed forward-looking cards — `TK-P3-04` (balance tuning: `rounds_per_match`/`match_end_grace_sec` are in-arena `@export` tunables), `TK-P3-06` (Charged Kick: a per-player, host-counted kick counter that resets on trigger — match-internal state), `TK-P4-*` (HUD/polish subscribing to `match_state_changed`/`tick_changed` within the arena) — none need match state to survive the scene transition. No tournament / multi-match / cross-match-scoring / persistent-session-stats feature exists anywhere on the current roadmap (backlog searched, none found). **Revisit trigger:** if such a cross-*match* persistence feature is ever scoped, the correct response is a NEW dedicated autoload for that cross-match state (or extending `NetworkManager`'s session ownership) — **not** promoting `GameManager`, whose scope is deliberately match-internal. Co-mingling cross-match state into GameManager would be the wrong boundary. This is an escalation-to-architect trigger, filed here so the decision isn't re-litigated silently.
- **Escalation note:** `gameplay-engineer` correctly did not need to escalate a *change* (there was none to make); `code-reviewer` correctly held the card on the *procedural* point that a flagged architecture question needs an explicit architect ruling on record before close. Both are right — this entry closes that procedural gap.
- **Result:** TK-P2-15 (and TK-P2-14, whose only architecture hold was a shared reference to this same scene-local question — TK-P2-14 has no autoload question of its own) are **cleared from the architect hold** to proceed to `qa-engineer` regression re-run, then the required human 2-instance pass (CASES A–E in TK-P2-15's `parked` note). Neither is flipped to `done` by architect — the human 2-instance pass still gates closure. No code changes in this entry.
